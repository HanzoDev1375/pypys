...
def env_vars(target_arch_name: str, android_api_level: int) -> Dict[str, str]:
    ...
    CLANG_PREFIX = (
        ndk_unified_toolchain() / f"{target_arch.ANDROID_TARGET}{android_api_level}"
    )
    ...
    env.update(
        {
            ...
            "LDFLAGS": f"-L{SYSROOT}/usr/lib -pie -lc++_shared",
        }
    )
...
def parse_args():
    ...
    parser.add_argument(
        "--arch", required=True, choices=ARCHITECTURES.keys(), dest="target_arch_name"
    )
    parser.add_argument(
        "--api",
        required=True,
